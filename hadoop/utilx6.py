...
class host:
	...
	def extract_csv_old(self, path, sql, output_dir='', script_path='', config={}, spark_args='', aux='', sep=',', header=False, remove='all'):
		# TODO other csv options: quote, escape, escapeQuotes, quoteAll, nullValue
		tmp_name = sha1(sql.encode()+self.prefix).hexdigest()[:16]
		...
		code = """
			...
			""".format(sql, output_dir, aux, '#RECYCLE', header, sep, app_config_str)
		...
		self.dfs('getmerge {0}/part* >{1} 2>{1}.log',output_dir,path)
