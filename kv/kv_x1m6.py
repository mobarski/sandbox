...
# licence: MIT
# version: x1m6 (x-experimental, p-preview, r-release, m-modification)
...
class KV:
	...
	def __init__(self, path=':memory:', zlevel=0, indexed=0, tab='main'):
		self.path = path
		self.tab = 'kv_'+tab # TODO sanitize?
	...
	def tables(self):
		...
		return [r[0][3:] for r in rows if r[0].startswith('kv_')]
	##
...
if __name__=="__main__":
	...
	for i in range(1): bench_kv(100000,'keys')
	#test_kv()
