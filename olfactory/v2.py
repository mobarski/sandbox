...
## boosting based on low activity
## circular input (255 is close to 0)
## categorical input (hash8(x) -> eq=255,ne=0)
## input activity monitoring
## input average used in learning
## change pernamence into its opposite (for target change) - avoid zeros
...
class onn:
	...
	"""
	def __init__(self, n_in, n_out, n_conn, p_lo=10, p_hi=100, sparse=False):
		self.cfg = {}
		self.cfg['n_in'] = n_in
	...
	def init(self):
		...
		if cfg['sparse']:
			# perm
			perm = np.random.randint(p_lo,p_hi,(n_out,2,n_conn),np.uint8)
			for i in conn:
				perm[i][0] = list(conn[i])
			self.perm = perm
			
			# targ
			targ = np.random.randint(0,255,(n_out,2,n_conn),np.uint8)
			for i in conn:
			...
			# targ
			targ = np.zeros((n_out,n_in),np.uint8)
			for i in conn:
	...
	def load(f):
		...
		if cfg['sparse']:
			...
			self.perm = np.fromfile(f, np.uint8,  n_out*n_in).reshape(n_out, n_in)
			self.targ = np.fromfile(f, np.uint8,  n_out*n_in).reshape(n_out, n_in)
		self.acti = np.fromfile(f, np.uint64, n_out)
		return self
	
	# ---[ CORE ]---------------------------------------------------------------
	
	# TODO e1 + e2 proportion
	...
	def learn(self, input, k, p_inc=10, p_dec=5, p_min=20, p_lo=21, p_hi=40):
		...
		for w in winners:
			...
			vt = targ[w][selected].astype(np.int16)
			vi = input[selected]
			delta = vt-vi
			error = abs(delta)
			...
			# update perm
			perm[w][sel[error < 10]] += p_inc
			perm[w][sel[error > 20]] += p_dec
