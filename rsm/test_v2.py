...
def test2():
	# data
	X1 = [set(combinations(1000,5)) for i in range(20)]
	X2 = [set(combinations(1000,5)) for i in range(20)]
	X3 = [set(combinations(1000,5)) for i in range(20)]
	# learning
	nn = rsm(50,5,0)
	for i in range(10):
		if 1 and i%3==0: # negative
			for x in X2:
				nn.learn(x,1,negative=True)
		for x in X1: # positive
			nn.learn(x,1,decay=0.0,dropout=0.0,quick=True)
			#print(nn.mem)
	# scoring
	m = 6
