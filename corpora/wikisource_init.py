...
if __name__=="__main__":
	...
	for col,url in url_list.items():
		col_title = unquote(get_title(url))
		#if col not in ['SC']: continue
		for i,local_url in enumerate(get_local_urls(url)):
			title = unquote(get_title(local_url))
			pos = i+1
			#if pos not in [75]: continue
			key = '{}{}'.format(col,pos)
			print(key,col,pos,title,end=' ')
			sys.stdout.flush()
			...
			# STORE
			wikisource.add_text(col,pos,title,col_title,local_url,text,aux)
		print()
