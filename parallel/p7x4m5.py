...
if sys.version < '2.7':
	...
	parser.add_option('-p',type=str,default='',help='output pipe command (TODO)')
	...
	parser.add_argument('-p',type=str,default='',help='ouput pipe command (TODO)')
...
for i in range(N):
	...
	proc = subprocess.Popen(cmd, stdin=PIPE, stdout=out_file, stderr=log_file, bufsize=BUFSIZE, shell=True)
	ctx[i]['proc'] = proc
	# metadata
	ctx[i]['pid'] = proc.pid
	ctx[i]['t_start'] = time()
