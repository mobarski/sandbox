...
function gif:frame(data)
	...
	self.file:write("\33\249\4\4\2\0\0\0") -- 50 fps -- TODO config.delay
	local last=self.last
	local x0, y0, x1, y1=0, nil, config.resolution[1]*config.scale-1, config.resolution[2]*config.scale-1
	if self.first then
		y0=0
		self.first=nil
	else
...
function giflib.new(filename,cfg)
	config.palette = cfg.palette
	config.palette2 = cfg.palette2
	...
	file:write("GIF89a"..num2str(config.resolution[1]*config.scale)..num2str(config.resolution[2]*config.scale).."\243\0\0")
	for i=1, 16 do
		local palette=config.palette2[i]
		file:write(string.char(palette[1], palette[2], palette[3]))
	end
	file:write("\33\255\11NETSCAPE2.0\3\1\0\0\0")
	local last=love.image.newImageData(config.resolution[1]*config.scale, config.resolution[2]*config.scale)
